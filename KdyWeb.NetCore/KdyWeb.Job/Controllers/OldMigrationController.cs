...
namespace KdyWeb.Job.Controllers
{
    ...
    public class OldMigrationController : OldBaseApiController
    {
        ...
        /// </summary>
        /// <returns></returns>
        [HttpGet("startSeries/{maxPage}")]
        public async Task<IActionResult> OldToNewSeries(int maxPage)
        {
            if (maxPage <= 0 || maxPage >= 200)
            {
                maxPage = 1;
            ...
            {
                var jobInput = new OldMigrationJobInput(page, pageSize);
                BackgroundJob.Enqueue<OldMigrationJobService>(a => a.OldToNewSeries(jobInput));
