...
namespace KdyWeb.Service.SequenceRecord
{
    ...
    public class GiftUserConfigService : BaseKdyService, IGiftUserConfigService
    {
        ...
        public async Task<KdyResult> CreateGiftUserConfigAsync(CreateGiftUserConfigInput input)
        {
            ...
            if (await _giftUserConfigRepository.ExistByUserIdAndGiftUserTypeAsync(input.UserId, input.GiftUserType))
            {
        ...
        {
            var dbEntity = await _giftUserConfigRepository.FirstOrDefaultAsync(a => a.Id == input.GiftUserConfigId);
            if (dbEntity == null)
            {
